...
3. 게임 규칙 상수
   1. 블랙잭 : 21
   2. 딜러 제외 최소, 최대 인원 : 1 ~ 7 (요구사항 외 임의로 정함)
   3. 최소, 최대 베팅 금액 : 5_000 ~ 10_000_000 (요구사항 외 임의로 정함)
4. 카드 지급기
...
### 도메인 로직
1.
   - [ ] 게임에 참여할 사용자를 모두 등록하는 기능
     - [ ] 플레이어 객체를 새로 생성하여 이름, 베팅 금액을 저장한다.
     - [ ] 정보를 저장한 플레이어 객체를 사용자 목록에 저장한다.
       - [ ] `예외` 참여 인원이 딜러를 제외하고 8명 이상일 경우 예외 발생
     - [ ] 딜러 객체를 하나 생성하여 사용자 목록에 저장한다.
2.
   - [ ] 무작위 순서로 섞여 있는 트럼프 카드를 세팅하는 기능
   - [ ] 사용자에게 카드를 지급하는 기능
     - [ ] 트럼프 카드의 가장 위에 있는 카드 한 장을 꺼낸다.
       - [ ] `예외` 트럼프 카드의 모든 카드를 소진했을 경우 예외 발생
     - [ ] 사용자의 정보에 그 카드를 저장한다.
3.
...
13.
    - [ ] 최종 수익을 출력하는 기능
        ...
        - [ ] `예외` 출력할 값이 null이면 예외 발생
14. - [ ] 사용자 입력 외 오류가 났을 때 이를 사용자에게 알리는 기능
