...
3. 
    - [x] 사용자에게 카드를 지급하는 기능
      - [x] 전달받은 인덱스를 가진 사용자의 정보에 덱에서 꺼낸 카드를 저장한다.
        - [x] `예외` 전달받은 인덱스 번호가 유효하지 않을 경우 예외 발생
...
9. 
   - [ ] 딜러, 플레이어의 수익을 더하고 빼는 기능
     - [ ] `예외` 존재하지 않는 이름의 사용자 수익을 업데이트하려고 할 경우 예외 발생
10.
    - [ ] 카드 상태에 따라 수익을 계산하는 기능
      - [ ] 오픈 후, 딜러와 플레이어의 블랙잭 여부에 따라 수익을 계산하는 기능
      - [ ] 플레이 후, 한 플레이어와 그에 대한 딜러의 버스트 / 승리 여부에 따른 수익을 계산하는 기능
...
3.
    - [x] 형식에 맞춰 입력 요청 및 진행 안내 메시지를 출력하는 기능
      ...
      - [x] 입력 요청: `(사용자명)는(/은) 한장의 카드를 더 받겠습니까?(예는 y, 아니오는n)`
...
6. 
   - [ ] 사용자 입력 오류가 났을 때 이를 알리고 다시 입력받는 기능
7. 
   - [ ] 사용자 입력 외 오류가 났을 때 이를 사용자에게 알리는 기능
...
- [x] Domain-3 : 사용자 카드 지급
  - [x] 전달받은 인덱스 번호가 유효하지 않을 경우 예외 발생
...
- [ ] Domain-9 : 수익 업데이트
  - [ ] 존재하지 않는 이름의 사용자 수익을 업데이트하려고 할 경우 예외 발생
- [ ] Domain-10 : 수익 계산
  - [ ] 블랙잭 여부에 따라 다르게 수익 계산
  - [ ] 각 플레이어의 푸쉬(동점), 승리, 패배, 버스트에 따라 다르게 수익 계산
  - [ ] 딜러가 버스트이면 버스트 제외한 플레이어 모두 베팅 금액만큼 수익 계산